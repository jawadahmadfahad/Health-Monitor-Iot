...
export const getArduinoSetupInstructions = (): string[] => {
  return [
    "1. Upload arduino/HealthMonitor.ino to an Arduino UNO/Nano",
    "2. Upload nodemcu/WebSocketBridge/WebSocketBridge.ino to a NodeMCU (ESP8266)",
    "3. Wire sensors: pulse sensor -> Arduino D7, LM35 -> Arduino A0, LCD -> (RS=12, E=11, D4=5, D5=4, D6=3, D7=2)",
    "4. Wire serial bridge: Arduino TX -> NodeMCU D1 (GPIO5) (use level shifting/resistor divider!), Arduino RX <- NodeMCU D2 (GPIO4) (optional), and connect GND-to-GND",
    "5. Update WiFi credentials in the NodeMCU sketch and read the IP address from the Serial Monitor",
    "6. Update the WebSocket URL in arduinoService.ts to ws://<NODEMCU_IP>:8080"
  ];
};
